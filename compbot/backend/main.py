        ...
        for attempt in range(max_retries):
            try:
                ...
                if response.status_code in [200, 201, 204]:
                    ...
                    logger.error(f"Database auth error {response.status_code}: {response.text[:200]}")
                    if attempt < max_retries - 1:
                        continue
                    raise HTTPException(
                        status_code=status.HTTP_401_UNAUTHORIZED,
                    ...
                    raise HTTPException(
                        ...
                        detail=f"Database error: {response.text[:200]}"
                    )
...
async def discord_auth_token(request: DiscordAuthRequest):
    ...
    try:
        ...
        token_data = {
            ...
            'code': request.code,
            'redirect_uri': request.redirect_uri or f"{Config.FRONTEND_URL}/callback"
        }
        ...
        if users:
            ...
            DatabaseService.insert("users", user_data, admin=True)
            users = DatabaseService.select("users", f"id=eq.'{user_id}'")
            user = users[0]
...
async def link_email(request: LinkEmailRequest, current_user: Dict = Depends(AuthService.get_current_user)):
        ...
        for i in range(0, len(team_list), 2):
            if i + 1 < len(team_list):
                ...
                match_data = {
                    ...
                    "team1_name": team1.get('name'),
                    "team2_name": team2.get('name'),
                    "region": team1.get('region', 'GLOBAL'),
                    "team1_score": 0,
                    "team2_score": 0,
                    "winner_id": None,
                ...
                DatabaseService.insert("matches", match_data, admin=True)
                matches.append(match_data)
...
async def register_team(
    ...
    try:
        ...
        # Check if tournament is accepting registrations
        if tournament.get('status') != TournamentStatus.REGISTRATION.value:
            raise HTTPException(
                status_code=400,
                detail="Tournament is not accepting registrations"
            )
        ...
        team_data = {
            ...
            "captain_name": current_user.get('username'),
            "region": request.region,
            "members": json.dumps(request.members or []),
            "status": "registered",
            "created_at": datetime.utcnow().isoformat()
        }
...
async def get_tournament_teams(tournament_id: str):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Error getting teams")

# ========== MATCH ENDPOINTS ==========
@app.get("/api/tournaments/{tournament_id}/matches")
async def get_tournament_matches(tournament_id: str, round_number: int = None):
    """Get matches for a tournament"""
    try:
        query = f"tournament_id=eq.'{tournament_id}'"
        if round_number:
            query += f"&round_number=eq.{round_number}"
        
        matches = DatabaseService.select("matches", query)
        return {
            "success": True,
            "matches": matches or [],
        ...
        raise HTTPException(status_code=500, detail="Error getting matches")
...
async def get_tournament_bracket(tournament_id: str):
    ...
    try:
        ...
        for round_num in sorted(rounds.keys()):
            ...
            for match in round_matches:
                # Find team details
                team1 = next((t for t in (teams or []) if t.get('id') == match.get('team1_id')), {})
                team2 = next((t for t in (teams or []) if t.get('id') == match.get('team2_id')), {})
...
async def root():
    ...
    return {
        ...
        "docs": "/docs",
        "health": "/api/health",
        "features": ["authentication", "tournaments", "teams", "brackets"]
    }
